...
const Healthcare = () => {
    ...
    const contractAddress = "0x4c189591ca46101fd5995ce60694f8c088837b00";
    ...
    const fetchPatientRecords = async () => {
        try {
            ...
            console.error("Error fetching FIR records", error);
        }
    }
    ...
    return(
        <div className='container'>
            <h1 className = "title">FIR REGISTRATION</h1>
            {account && <p className='account-info'>Connected Account: {account}</p>}
            {isOwner && <p className='owner-info'>You are the contract owner</p>}

        <div className='form-section'>
            <h2>Fetch FIR Records</h2>
            <input className='input-field' type='text' placeholder='Enter FIR ID' value={patientID} onChange={(e) => setPatientID(e.target.value)}/>
            <button className='action-button' onClick={fetchPatientRecords}>Fetch Records</button>
        </div>

        <div className="form-section">
            <h2>Crime And Description Records</h2>
            <input className='input-field' type='text' placeholder='Crime' value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)}/>
            <input className='input-field' type='text' placeholder='Description' value={treatment} onChange={(e) => setTreatment(e.target.value)}/>
            <button className='action-button' onClick={addRecord}>Add Records</button>

        </div>
        <div className="form-section">
            <h2>Authorized FIR Provider</h2>
        ...
        <div className='records-section'>
            ...
            {patientRecords.map((record, index) => (
                <div key = {index}>
                    ...
                    <p>Crime: {record.diagnosis}</p>
                    <p>Description: {record.treatment}</p>
